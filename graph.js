...
function chamber_seats(which_chamber, how_many) {
    ...
    if (which_chamber == HOUSE) {
        rows = [ 22, 25, 31, 34, 38, 41, 43, 45, 47, 53, 56];
        c_r = 7.5;
        d_r = 22;
        r = 130;
    } else {
    ...
    for (var i=0; i<rows.length && seat_i<how_many; i++) {
        let theta = 0;
        let n = rows[i];
        for (let j=0; j<n && seat_i<how_many; j++) {
            let x = origin_x + r*degcos(theta);
            let My = origin_My + r*degsin(theta);
    ...
    for (let i=0; i<members.length; i++) {
        if (members[i][1] === "House") {
            ...
            if (member[MEM.district_code] == 0) {
                ...
                let district_code = member[MEM.state]+member[MEM.district_code].toString();
...
function senate_seat_groups(members) {
    ...
    for (let i=0; i<members.length; i++) {
        if (members[i][1] === "Senate") {
            let member = members[i];
            let icspr = member[MEM.icspr];
            let state = member[MEM.state];
            if (states[state]) {
...
function assign_seats(seat_groups, chamber_seats) {
    let result = {};
    let seat_i = 0;
    for (let i=0; i<seat_groups.length; i++) {
...
function init_chamber(which_chamber, rollcalls, votes, members) {
    ...
    let seat_groups;
    if (which_chamber==HOUSE) {
        seat_groups = house_seat_groups(members);
    } else {
        seat_groups = senate_seat_groups(members);
    }
...
function rollcall_clicked(event) {
    ...
    reset_vote(st.chamber);
    if (st.cur_button) {
        unexpand_button(st.cur_button);
    }
    let vote_n = Number(button.getAttribute("vote-number"));
    st.chamber.rollcall = vote_n;
...
function congress_or_chamber_selector_opened(event) {
    st.prevent_hover = true;
}

function congress_or_chamber_selector_closed(event) {
    st.prevent_hover = false;
}

function congress_or_chamber_selected(event) {
    const select_congress = document.querySelector("#select-congress");
...
function setup_congress_selector(initial_congress_n) {
    ...
    selector.addEventListener("change", congress_or_chamber_selected);
    selector.addEventListener("focus", congress_or_chamber_selector_opened);
    selector.addEventListener("blur", congress_or_chamber_selector_closed);
}

function search_rollcalls(ev)
{
    const query = ev.target.value;
    rollcall_table(st.chamber.rollcalls, st.chamber.which, query);
}
...
function congress_main(members_str, rollcalls_str, votes_buf, congress_n, chamber) {
    ...
    const select_chamber = document.querySelector("#select-chamber");
    select_chamber.addEventListener("change", congress_or_chamber_selected);
    select_chamber.addEventListener("focus", congress_or_chamber_selector_opened);
    select_chamber.addEventListener("blur", congress_or_chamber_selector_closed);

    load_rollcall(st.chamber, st.chamber.rollcall);
